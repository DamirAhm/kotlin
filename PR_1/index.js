...
(async () => {
    ...
    if (searchResults.length === 0) {
        console.log('К сожалению по вашему запросу не найдено ни одной страницы на википедии');
    ...
    const selectedItem = searchResults.find(({title}) => title === selectedOption);

    if (selectedItem) {
        await open(WikipediaService.getArticleUrl(selectedItem.pageid));
    } else {
        console.log('Во время выполнения программы произошла ошибка, выбрана несуществующий вариант ответа');
    }
})();